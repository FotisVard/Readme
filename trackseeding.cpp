...

{
    ...
    //enum chambers{ ARTEMIS_X, APOLLON_Y, BEATRICE_X, CASTOR_Y, CLEO_X , DIONYSOS_Y, DAPHNE_X, BYRON_Y };
    map< string, chambers> enumResolver;
...
float       corr_pos1X= 0.27,   corr_pos2X= -0.37,  corr_pos3X= 0.24 ,       corr_pos4X = 0.0;
//float       corr_pos1Y= 0.00 ,  corr_pos2Y= 0.000 , corr_pos3Y= 0.00 ,     corr_pos4Y = 0.00;
    ...
    // CUTS
    int cl_minstrp=0 ,cl_maxstrp=9;  // min and max cluster size (in strips)
...
// =========================================================================
            ...
            // This lambda does a lightning-fast mathematical line fit, finds the worst residual,
            // and swaps in other hits if the residual exceeds 10.0 mm to see if it improves!
            auto optimizeHitSelection = [&](vector<MMCluster*>& v1, vector<MMCluster*>& v2, vector<MMCluster*>& v3, vector<MMCluster*>& v4) {
                std::vector<int> sel = {0, 0, 0, 0}; // Start by guessing the highest-charge clusters
                ...
                auto evalFit = [&](int i1, int i2, int i3, int i4, double& max_res, int& worst_layer) {
                    ...
                    // Fast Ordinary Least Squares (OLS) Fit
                    ...
                    max_res = -1.0;
                    worst_layer = -1;
                    // Find the layer with the largest deviation from the fitted line
                    for(int i=0; i<4; ++i) {
                ...
                // ITERATIVE SWAPPING: If a hit is off by >10mm, test the other hits in that layer!
                int iterations = 0;
                while (current_max_res > 10.0 && current_worst_layer != -1 && iterations < 4) {
                    ...
                    // Determine how many alternative clusters exist in the bad layer
                    int num_clusters = 0;
                    ...
                    // Test every other cluster in this layer
                    for (int j = 0; j < num_clusters; ++j) {
                        ...
                        // If swapping this hit improved the overall track error, save it!
                    ...
                    if (improved) {
                        ...
                        // Recalculate to see if a different layer is now the worst one
                        evalFit(sel[0], sel[1], sel[2], sel[3], current_max_res, current_worst_layer);
                    } else {
                        // If no other hit improved the track, break out to avoid infinite loops
                        break; 
                    }
                    iterations++;
                }
            ...
            // Run the algorithm to find the absolute best hit indices for X and Y!
            ...
            // Assign the final, optimized simple centroid positions
...
// =========================================================================
// --- UPGRADED CHARGE-WEIGHTED uTPC WITH GEOMETRIC SAFETY ---
// =========================================================================
auto computeUtpcPosition = [&](MMCluster* cluster) {
    if (cluster == 0) return 0.0;
    ...
    double Sx = 0.0;   // Sum of X (strip position)
    double Szz = 0.0;  // Sum of Z^2
    double Szx = 0.0;  // Sum of Z*X

    for (int i = 0; i < hitCount; ++i) {
        double x = (cluster->Get(i).Strip() + 0.5) * pitch_val;
        double z = cluster->Get(i).gety(); 
        double w = cluster->Get(i).Charge();

        // Apply the delta-electron cap to the weight!
        // This stops single massive spikes from acting as a heavy anchor.
        if (w > charge_cap) w = charge_cap;
    ...
    // Safety: If denom is ~0, the track is perfectly vertical (all hits arrived at same time).
    // A line cannot be drawn. Fallback to centroid!
    if (fabs(denom) < 1e-6) {
        return getCappedPos(cluster, pitch_val, charge_cap); 
    }

    // Fit X as a function of Z (Drift): X(Z) = m*Z + c
    double m_utpc = ((Sw * Szx) - (Sz * Sx)) / denom;
    double c_utpc = (Sx - m_utpc * Sz) / Sw;

    // Evaluate X at the exact middle of the 5.0 mm drift gap (Z = 2.5 mm)
    double utpcPos = (m_utpc * 2.5) + c_utpc;

    // --- THE EXTRAPOLATION SAFETY NET ---
    // If the timing data is highly distorted by noise, the line might extrapolate 
    // the hit completely outside the physical bounds of the cluster.
    double centroidPos = getCappedPos(cluster, pitch_val, charge_cap);
    double clusterWidth = hitCount * pitch_val;
    
    // If the uTPC throws the coordinate further away than the cluster itself is wide,
    // the timing data is trash. Trust the charge centroid instead!
...
*/
            ...
            auto getDynamicResX = [&](MMCluster* cluster) {
                ...
                if (size == 1) return 0.680448; // If only 1 strip, it's likely a grazing track with a tiny cloud. Assign a large error to reflect the uncertainty.
                //if (size == 1) return 3; 

                if (size == 2) return 0.622183; // Charge sharing yields excellent resolution!
                if (size == 3) return 0.59619; // 3-strip clusters are wider, so assign a worse resolution.
                if (size == 4) return 0.59619;
                if (size == 5) return 0.659963;
                if (size == 6) return 0.797531;
                if (size == 7) return 0.962209;
            ...
            // Calculate the specific error for each of the 4 X layers
            float errX1 = getDynamicResX(VClustersX1.at(0));
            float errX2 = getDynamicResX(VClustersX2.at(0));
            float errX3 = getDynamicResX(VClustersX3.at(0));
            float errX4 = getDynamicResX(VClustersX4.at(0));

            auto getDynamicResY = [&](MMCluster* cluster) {
                //if (cluster == 0) return 0.060;
                ...
                //sq = 16 adc
                // NEW DIMENSION: Size 1 means electron cloud stayed inside the safe zone!
                if (size == 1) return 0.689965; // If only 1 strip, it's likely a grazing track with a tiny cloud. Assign a large error to reflect the uncertainty.
                //if (size == 1) return 3; 

                if (size == 2) return 0.678178; // Charge sharing yields excellent resolution!
                if (size == 3) return 0.693365; // 3-strip clusters are wider, so assign a worse resolution.
                if (size == 4) return 0.693365;
                if (size == 5) return 0.762661;
                if (size == 6) return 0.900457;
                if (size == 7) return 1.06574;
                if (size == 8) return 1.39;
                if (size == 9) return 1.57;
            };

            // Calculate errors for the 4 Y layers
            float errY1 = getDynamicResY(VClustersY1.at(0));
            float errY2 = getDynamicResY(VClustersY2.at(0));
            float errY3 = getDynamicResY(VClustersY3.at(0));
            float errY4 = getDynamicResY(VClustersY4.at(0));
            
            float z_err[4] = {0.0, 0.0, 0.0, 0.0}; 
            float xy_err[4] = {errX1, errX2, errX3, errX4}; // Fed into graph0
...
delete graph0;
            ...
            // for Y
            TGraphErrors* graph1 = new TGraphErrors(4, zpos, ypos, z_err, xy_err);
            trackfit->SetParameters(0.0, 0.0);
